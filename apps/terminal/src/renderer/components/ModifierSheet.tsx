  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6">
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        {/* Header */}
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-gray-200 px-8 py-5">
          <div className="min-w-0">
            <h2 className="truncate text-3xl font-bold text-gray-900">{product.name}</h2>
            {product.description && (
              <p className="mt-1 line-clamp-2 text-sm text-gray-500">{product.description}</p>
            )}
            <p className="mt-1 text-xl font-bold text-primary-600">{money(unitPrice)}</p>
          </div>
          <button
            onClick={onClose}
            aria-label="Close"
            className="pos-btn-secondary h-14 w-14 shrink-0"
          >
            <X className="h-7 w-7" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-2">
            ...
            return (
              <section key={group.id} className="py-4">
                <div className="flex items-baseline justify-between gap-3">
                  <h3 className="text-xl font-bold text-gray-900">{group.name.trim()}</h3>
                  <span
                  ...
                  {group.modifiers.map((modifier) => {
                    ...
                    return (
                      <button
                        ...
                        className={`pos-btn min-h-[72px] justify-between gap-2 border-2 px-4 text-left ${
                          isSelected
                            ? 'border-primary-500 bg-primary-50 text-primary-700'
                            : blocked
                              ? 'cursor-not-allowed border-gray-200 bg-gray-50 text-gray-400'
                              : 'border-gray-300 bg-white text-gray-800 hover:border-primary-300'
                        }`}
                      >
                        ...
                        {modifier.price > 0 && (
                          <span className="shrink-0 text-sm font-bold">+{money(modifier.price)}</span>
                        )}
                      </button>
                    );
                  })}
          ...
          <div className="py-4">
            ...
            <textarea
              ...
              rows={2}
              className="w-full resize-none rounded-lg border-2 border-gray-300 px-4 py-3 text-base focus:border-primary-500 focus:outline-none"
            />
          </div>
        </div>

        {/* Footer */}
        <div className="shrink-0 border-t border-gray-200 bg-gray-50 px-8 py-5">
          {unmetGroups.length > 0 && (
            <div className="mb-3 flex items-center gap-2 rounded-lg bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">
          ...
          <div className="flex items-center gap-5">
            <div className="flex items-center gap-1 rounded-xl border-2 border-gray-300 bg-white p-1">
              <button
                ...
                className="pos-btn h-16 w-16 text-gray-700 disabled:opacity-30"
              >
                <Minus className="h-7 w-7" strokeWidth={3} />
              </button>
              <span className="w-12 text-center text-2xl font-bold" aria-live="polite">
                {quantity}
              </span>
              <button
                onClick={() => setQuantity((q) => Math.min(99, q + 1))}
                aria-label="Increase quantity"
                className="pos-btn h-16 w-16 text-gray-700"
              >
            ...
            <button
              onClick={handleAdd}
              className={`flex-1 justify-between px-8 text-lg ${
                unmetGroups.length > 0
